...
const editProduct = async () => {
  loading.value = true;
  try{
    await $axios.put(`/products/${route.params.id}`, { name: dataLocal.value.name, image: dataLocal.value.image, category_id: dataLocal.value.category.id})
  ...
  <VRow>
    <VCol cols="12">
     <VBtn to="/products">
      ...
      >
        <!-- 👉 Horizontal Form -->
        <VCard :loading="loading" title="Editar Produto">
          <VCardText>
 ...
 <!-- 👉 Email -->
    ...
    single-line
      ...
      <VCol
        ...
        class="d-flex gap-4"
      >
        <VBtn :disabled="loading" @click="editProduct">
          Editar
        </VBtn>
        <VBtn
          color="secondary"
